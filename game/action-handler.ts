        ...
        switch (event.soloValue) {
          case 'right': {
            ...
            updateLevelState((s) => {
              ...
              if (isValid) {
                deploy(ticket, zone.element, controller.id)
              } else {
                state.bugs[controller.id] += Math.ceil(
